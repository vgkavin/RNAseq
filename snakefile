...
#to trim the nexteratransposase adapters(optional)
rule nextera_trim:
    input:
...
#to check the quality of trimmed reads
rule fastqc:
    input:
...
#to ensure the sample are free of contaminants
rule fastq_screen:
    input:
        read1=rules.nextera_trim.output.trimmed_read1,
	read2=rules.nextera_trim.output.trimmed_read2
...
#generate counts of gene transcripts in each sample
rule count:
    input:
...
#(optional)consolidate all qc and counts results
rule multiqc:
    input:
        expand(config["dirnames"]["fastqc_dir"]+ "/{sample_id}.nextera.R1_fastqc.html", sample_id=sample_ids),
        expand(config["dirnames"]["fastqc_dir"]+ "/{sample_id}.nextera.R2_fastqc.html", sample_id=sample_ids),
